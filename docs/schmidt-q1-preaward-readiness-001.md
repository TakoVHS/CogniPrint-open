...
| 3–6-principal fixtures | SYNTHETIC_3P_4P_6P_COVERAGE | three fixtures under `challenge-schmidt-q1/fixtures/` | replace/extend with realistic frozen scenario families and hidden ground truth |
...
The repository branch should be executed from an exact clean checkout. GitHub Actions on this repository currently returns created jobs with no executed steps/logs, so no hosted PASS is claimed. A non-GitHub local runner may provide development evidence, but it must not be represented as exact-head GitHub Actions evidence. Existing repository governance around execution truth remains in force.
